...
private fun ConfirmPane(
    ...
    OverlayCard(
        ...
        extra = if (quote.isNotBlank()) {
            {
                val appleQuote = LocalVisualStyle.current == VisualStyle.APPLE
                val darkQuote = LocalDarkTheme.current
                Surface(
                    modifier = if (appleQuote) Modifier.peaceGlass(RoundedCornerShape(18.dp), darkQuote, true) else Modifier,
                    color = if (appleQuote) Color.Transparent
...
private fun OverlayCard(
    ...
    Surface(
        ...
        color = if (apple) Color.Transparent
